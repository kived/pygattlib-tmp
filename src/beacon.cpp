...
BeaconService::start_advertising(const std::string uuid, int major, int minor,
        int txpower, int interval, int companyid, int prefix1, int prefix2) {
    ...
    if(!(-80 <= txpower && txpower <= 4)) {
        throw std::runtime_error("Incorrect txpower value(must be: -80 to 4)");
    }
    ...
    beacon_data->company_id = htobs(companyid);
    beacon_data->type = htobs(prefix1 & 0xFF);
    beacon_data->data_len = htobs(prefix2 & 0xFF);
...
//    adv_data_cp.data[adv_data_cp.length] = htobs(prefix & 0xFF);
//    adv_data_cp.length += htobs(segment_length - 1 + sizeof(beacon_adv) - 1) + 1;
